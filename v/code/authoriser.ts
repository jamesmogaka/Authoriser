...
export class authoriser extends view {
  ...
  async authorise(event: Event): Promise<void> {
    ...
    //
    //Return to the user to the caller by closing this window
    this.win.close();
    */
    this.mywindow!.close_auth();
  }

  //Adminitster te authorisation process from a caller application
  async administer():Promise<void>{
    //
    //Create a new window
    this.mywindow = window.open(this.url,'_blank','fullscreen=yes');
    //
    //Abort if the openning failed
    if (!this.mywindow) throw new mutall_error(`Window opening failed`);
    //
    //Wait for the baby window to close
    const user:user = await new Promise(resolve=>{
      ...
      //
      //Add the closing function
      this.mywindow!.onbeforeunload = ()=>{
